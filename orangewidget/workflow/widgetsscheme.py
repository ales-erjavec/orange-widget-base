...
class WidgetsSignalManager(SignalManager):
    ...
    def send(self, widget, channelname, value, *args, **kwargs):
        ...
        def _signal_id(signal_id):
            return signal_id
        extra_args = ()
        try:
            signal_id = _signal_id(*args, **kwargs)
        except TypeError:
            pass
        else:
            warnings.warn(
            ...
            extra_args = (signal_id,)

        super().send(node, channel, value, *extra_args)
...
class WeakKeyDefaultDict(WeakKeyDictionary):
    """
    A `WeakKeyDictionary` that also acts like a :class:`collections.defautdict`
    """
